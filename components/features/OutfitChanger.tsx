...
const OutfitChanger: React.FC<OutfitChangerProps> = ({ sessionToLoad, onSaveSession, imageToLoad, onSendImage }) => {
  ...
  const handleOutfitImageUpload = (image: string) => {
    if (options.imageValue.length < 4) {
      setOptions(o => ({ ...o, imageValue: [...o.imageValue, image] }));
    }
  };
    ...
    try {
      ...
      for (let i = 0; i < numberOfImages; i++) {
        ...
        if (options.type === 'prompt') {
          newImage = await changeOutfitWithPrompt(baseImage, options.promptValue);
        } else {
          newImage = await changeOutfitWithImage(baseImage, options.imageValue);
        }
      ...
      onSaveSession({
        ...
        parameters: { options, numberOfImages },
      });
    } catch (e) {
