...
import { generateAffiliateImage, generateProductBackgroundImage, editImageWithPrompt, suggestProductBackgrounds, suggestPrompts } from '../../services/geminiService';
...
const AffiliateImageGenerator: React.FC<AffiliateImageGeneratorProps> = ({ sessionToLoad, onSaveSession, imageToLoad, onSendImage }) => {
  const [mode, setMode] = useState<'kol' | 'product'>('kol');
  ...
  const handleProductImageUpload = (image: string) => {
    if (products.length < 4) {
      setProducts(prev => [...prev, { image, name: '' }]);
    }
  };
  ...
  useEffect(() => {
    if (imageToLoad) {
      // Default to adding the image as a product in product mode, or KOL in KOL mode
      if (mode === 'product') {
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        if (mode === 'kol') {
            ...
            sessionParams = { mode, prompt, modelImages, products, aspectRatio, quality, numberOfImages, imageType };
            for (let i = 0; i < numberOfImages; i++) {
                setLoadingMessage(`Đang tạo ảnh ${i + 1} trên ${numberOfImages}...`);
                const newImage = await generateAffiliateImage(modelImages, products, prompt, aspectRatio, quality, imageType);
            ...
            sessionParams = { mode, prompt, modelImages: [], products, backgroundImage, aspectRatio, quality, numberOfImages, imageType };
             for (let i = 0; i < numberOfImages; i++) {
                setLoadingMessage(`Đang tạo ảnh ${i + 1} trên ${numberOfImages}...`);
                const newImage = await generateProductBackgroundImage(products, prompt, backgroundImage, aspectRatio, quality, imageType);
                results.push(newImage);
            }
        }
      ...
      onSaveSession({
        originalImage: (mode === 'kol' ? modelImages[0] : products[0]?.image) || '',
  ...
  return (
    <>
      <FeatureContainer
        ...
        description="Chọn chế độ để tạo ảnh review với KOL hoặc tạo ảnh quảng cáo cho sản phẩm."
        onSubmit={handleSubmit}
        isLoading={isLoading}
        canSubmit={canSubmit}
      >
        <div className="flex bg-slate-700 rounded-lg p-1 mb-6">
            <button onClick={() => setMode('kol')} className={`w-1/2 py-2 rounded-md transition-colors ${mode === 'kol' ? 'bg-purple-600 text-white' : 'text-gray-300 hover:bg-slate-600'}`}>
                KOL và Sản phẩm
            </button>
            <button onClick={() => setMode('product')} className={`w-1/2 py-2 rounded-md transition-colors ${mode === 'product' ? 'bg-purple-600 text-white' : 'text-gray-300 hover:bg-slate-600'}`}>
                Thay nền sản phẩm
            </button>
        </div>
        ...
        {mode === 'kol' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                    ...
                    {/* Product Uploader */}
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">2. Tải ảnh & đặt tên sản phẩm ({products.length}/4)</label>
                        {products.length > 0 && (
                        ...
                        <input type="file" ref={productFileInputRef} onChange={(e) => handleFileSelect(e, 'product')} className="hidden" accept="image/*"/>
                        {products.length < 4 && <button onClick={() => productFileInputRef.current?.click()} className="w-full py-3 border-2 border-dashed border-slate-600 rounded-lg flex justify-center items-center text-center cursor-pointer hover:border-purple-400 hover:bg-slate-800/50 transition-colors text-gray-300">Thêm sản phẩm</button>}
                    </div>
                </div>
                <div className="space-y-6">
                    <div>
                        <label htmlFor="prompt-kol" className="block text-sm font-medium text-gray-300 mb-2">3. Mô tả tương tác</label>
                        <div className="relative">
                            <textarea id="prompt-kol" value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Ví dụ: KOL đang cầm chai nước hoa và mỉm cười, bối cảnh sang trọng với ánh đèn vàng" rows={8} className="w-full bg-slate-700 border border-slate-600 rounded-md p-3 text-white focus:ring-purple-500 focus:border-purple-500"/>
        ...
        {mode === 'product' && (
             <div className="space-y-6">
                 <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">1. Tải ảnh & đặt tên sản phẩm ({products.length}/4)</label>
                    {products.length > 0 && (
                        <div className="space-y-4 mb-2 p-2 bg-slate-900/50 rounded-lg">{products.map((p, i) => (<div key={i} className="flex items-center gap-3"><div className="relative group w-20 h-20 flex-shrink-0"><img src={p.image} alt={`Product ${i+1}`} className="w-full h-full object-cover rounded-md"/><button onClick={() => handleRemoveProduct(i)} className="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`Xóa sản phẩm ${i+1}`}><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg></button></div><input type="text" value={p.name} onChange={(e) => handleProductNameChange(i, e.target.value)} placeholder={`Tên sản phẩm ${i+1}`} className="w-full bg-slate-700 border border-slate-600 rounded-md p-2 text-sm text-white focus:ring-purple-500 focus:border-purple-500"/></div>))}</div>
                    )}
                    <input type="file" ref={productFileInputRef} onChange={(e) => handleFileSelect(e, 'product')} className="hidden" accept="image/*"/>
